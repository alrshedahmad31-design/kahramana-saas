...
import InventoryAlertsListener from '@/components/inventory/InventoryAlertsListener'
...
export default async function DashboardLayout({ children, params }: Props) {
  ...
  const showInventoryAlerts = canAccessSection(user.role, 'inventory')
  ...
  return (
    <div className="-mt-20 md:-mt-24 min-h-screen bg-brand-black flex" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* Realtime inventory alerts — shown for inventory-access roles */}
      {showInventoryAlerts && <InventoryAlertsListener />}
    </div>
  )
}
