    ...
    <div className="flex flex-col gap-6">
      ...
      {branchSummary.length === 0 ? (
        <div className="border border-brand-border rounded-xl p-12 text-center">
          <p className="font-cairo text-brand-muted">
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-satoshi text-lg font-black text-brand-text">
              ...
              <div className="border border-brand-border rounded-xl overflow-hidden overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-brand-surface-2">
                    <tr>
                      <th className="px-4 py-3 text-start font-cairo text-xs text-brand-muted uppercase tracking-wide">{t('branch')}</th>
                      <th className="px-4 py-3 text-start font-cairo text-xs text-brand-muted uppercase tracking-wide">{t('category')}</th>
                      <th className="px-4 py-3 text-start font-cairo text-xs text-brand-muted uppercase tracking-wide">{isAr ? 'العدد' : 'Count'}</th>
                      <th className="px-4 py-3 text-start font-cairo text-xs text-brand-muted uppercase tracking-wide">{t('totalValue')}</th>
                      <th className="px-4 py-3 text-start font-cairo text-xs text-brand-muted uppercase tracking-wide">{t('reserved')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {valuation.map((row, i) => (
                      <tr key={i} className="border-t border-brand-border hover:bg-brand-surface-2 transition-colors">
                        <td className="px-4 py-3 font-cairo text-sm text-brand-text">
                          {isAr ? row.branch_name_ar : row.branch_name_en}
                        </td>
                        <td className="px-4 py-3 font-cairo text-sm text-brand-muted">
                          {row.category ?? (isAr ? 'غير محدد' : 'Uncategorized')}
                        </td>
                        ...
                        <td className="px-4 py-3 font-satoshi text-sm font-medium text-brand-gold">
                          {Number(row.total_value_bhd).toFixed(3)}
                        </td>
                        <td className="px-4 py-3 font-satoshi text-sm text-brand-muted">
                          {Number(row.reserved_value_bhd).toFixed(3)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
