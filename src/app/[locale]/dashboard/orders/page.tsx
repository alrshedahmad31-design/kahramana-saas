...
// Map filter key → supabase statuses
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data } = await supabase
        ...
        .eq('id', user.id)
        .single()
      setUserRole((data as { role: StaffRole } | null)?.role ?? null)
    })
  }, [supabase])
  ...
  // Kanban: group current orders into 4 columns (no page limit — show all)
  ...
  return (
    <div className="flex flex-col gap-5" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* Stats bar — always visible, live counts for today */}
      <OrderStatsBar />
