...
// upper-cased). We reverse the lookup by matching the customer_profiles.id
// prefix. Six hex chars = ~16M combos so collisions are theoretically possible;
// if the customer base ever passes that bar this needs a dedicated membership_id
// column with a UNIQUE index — see DEFERRED note at bottom of this file.
//
// RLS on customer_profiles excludes 'waiter'/'cashier' (migration 008 limits
// staff_read_customer_profiles to owner/GM/BM/marketing). For this scaffold we
// gate access at the app layer via WAITER_ROLES and use the service-role
// client, mirroring how createWaiterOrder above handles table validation.
...
export type MemberLookupResult =
  ...
  | { ok: false; code: 'unauthorized' | 'rate_limited' | 'invalid_format' | 'not_found' | 'ambiguous' | 'lookup_failed' }
...
export async function lookupMemberByQR(membershipId: string): Promise<MemberLookupResult> {
  ...
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY
  if (!url || !key) return { ok: false, code: 'lookup_failed' }
  const supabase: SupabaseClient = createSupabaseClient(url, key, {
    auth: { persistSession: false, autoRefreshToken: false },
  })
...
// DEFERRED — when this flag is flipped on for real, follow up with:
//   1. ALTER TABLE customer_profiles ADD COLUMN membership_id TEXT
//      GENERATED ALWAYS AS ('KAH-' || upper(substring(replace(id::text,'-','') from 1 for 6))) STORED;
//      CREATE UNIQUE INDEX ON customer_profiles (membership_id);
//      → eliminates the 6-hex collision risk + lets us drop the ::text cast filter.
//   2. Extend the staff_read_customer_profiles RLS policy to waiter/cashier OR
//      ship a SECURITY DEFINER RPC `rpc_lookup_member_by_qr(p_id text)` so we can
//      use createClient() (RLS) instead of the service-role bypass above.
