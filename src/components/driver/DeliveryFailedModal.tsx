...
export default function DeliveryFailedModal({ orderId, orderRef, isRTL, onClose }: Props) {
  ...
  async function handleSubmit() {
    ...
    const result = await reportDeliveryFailure(orderId, selected, notes || undefined)
    setBusy(false)
  ...
  return (
    ...
    >
      ...
      {/* Bottom sheet */}
      <div className="relative w-full max-w-lg bg-brand-surface border-t border-brand-error/40 rounded-t-3xl overflow-hidden shadow-2xl">
          ...
          {done ? (
            <div className="flex flex-col items-center gap-3 py-6">
              ...
              <p className={`text-sm text-brand-muted text-center ${isRTL ? 'font-almarai' : 'font-satoshi'}`}>
                {isRTL ? 'تم تحويل الطلب للمدير للمراجعة' : 'Order escalated to manager for review'}
              </p>
            </div>
          ) : (
            <>
              ...
              <div>
                <p className={`text-xs font-bold text-brand-muted uppercase tracking-wider mb-3 ${isRTL ? 'font-almarai' : 'font-satoshi'}`}>
                  {isRTL ? 'سبب عدم التوصيل (مطلوب)' : 'Reason for failure (required)'}
                </p>
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`
                    ...
                    focus:outline-none focus:border-brand-error/50
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  `}
                >
                  {busy ? '…' : (isRTL ? 'تأكيد فشل التوصيل' : 'Confirm Failure')}
                </button>
              </div>
            </>
          )}
...
function CheckBigIcon() {
  return (
    <svg width={28} height={28} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="text-brand-error" aria-hidden="true">
