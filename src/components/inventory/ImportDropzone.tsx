    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {(phase === 'idle' || phase === 'selected') && (
            ...
            >
              ...
              <div className="text-center">
                <p className="font-cairo text-sm font-medium text-brand-text">
                  {t('dropzone')}
                </p>
                <p className="font-satoshi text-xs text-brand-muted mt-1">
                  {locale === 'ar' ? 'الحد الأقصى: 10 MB' : 'Max size: 10 MB'}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex items-center gap-3 rounded-lg bg-brand-surface-2 px-4 py-3">
              ...
              {phase === 'selected' && (
                <button
                  type="button"
                  onClick={handleReset}
                  className="font-cairo text-xs text-brand-muted hover:text-brand-error transition-colors"
                >
          ...
          {phase === 'selected' && (
            <button
              ...
              className="self-start flex items-center gap-2 rounded-lg px-5 py-2.5
                bg-brand-gold text-brand-black font-cairo text-sm font-semibold
          ...
          {phase === 'analyzing' && (
            <div className="flex items-center gap-3">
              ...
              <span className="font-cairo text-sm text-brand-muted">
                {t('uploading')}
              </span>
            </div>
          )}
      ...
      {phase === 'done' && importSuccess && (
        <div className="rounded-xl border border-brand-success/30 bg-brand-success/5 p-5 flex flex-col gap-4">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            >
              {locale === 'ar' ? 'عرض المخزون' : 'View Inventory'}
            </a>
            <button
              type="button"
              onClick={handleReset}
              className="rounded-lg px-4 py-2.5 border border-brand-border
                font-cairo text-sm text-brand-muted hover:bg-brand-surface-2 transition-colors"
            >
              {locale === 'ar' ? 'استيراد آخر' : 'Import Another'}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-brand-border bg-brand-surface p-5 flex items-center gap-4">
        <div className="flex-1">
          <h3 className="font-cairo text-sm font-semibold text-brand-text">
            {locale === 'ar' ? 'تصدير البيانات الحالية' : 'Export Current Data'}
          </h3>
          <p className="font-cairo text-xs text-brand-muted mt-0.5">
            {locale === 'ar'
