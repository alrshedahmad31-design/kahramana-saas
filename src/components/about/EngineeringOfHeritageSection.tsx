...
export function EngineeringOfHeritageSection({ isRTL }: EngineeringOfHeritageSectionProps) {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative w-full py-24 md:py-32 bg-brand-black text-white overflow-hidden"
    >
      {/* Background Deep Gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-brand-black via-brand-surface to-brand-black opacity-60 pointer-events-none" />
      ...
      >
        ...
        <div className="max-w-4xl mx-auto text-center px-4 mb-16 md:mb-24">
          <SectionHeader
            ...
            subtitle={t('eyebrow')}
          />
          <p className="text-xl md:text-2xl text-brand-text mb-8 leading-relaxed font-light">
            {t('intro')}
          </p>
          <div className="w-32 h-px bg-brand-gold/40 mx-auto mb-8" />
        ...
        <div className="flex flex-col items-center mb-40 px-4 md:px-8">
          <div className="relative w-full max-w-[1600px] aspect-[16/9] md:aspect-[21/9] group mb-12 bg-brand-black/40 rounded-[2.5rem] border border-white/10 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)]">
            <Image
            ...
            <div className="absolute inset-0 z-20">
              {HOTSPOTS.map((spot) => (
                ...
                >
                  <div className={`relative flex items-center justify-center w-full h-full rounded-full transition-all duration-500 ${activeHotspot === spot.id ? 'bg-brand-gold/20 scale-125' : 'bg-transparent hover:bg-brand-gold/10'}`}>
                    <div className={`w-3.5 h-3.5 md:w-5 md:h-5 rounded-full border-2 border-brand-gold transition-all duration-300 ${activeHotspot === spot.id ? 'bg-brand-gold scale-110 shadow-[0_0_20px_rgba(200,146,42,1)]' : 'bg-brand-black shadow-xl'}`} />
                    {activeHotspot === spot.id && (
                      <motion.div layoutId="blueprintPulse" className="absolute inset-0 rounded-full border border-brand-gold animate-ping opacity-40" />
                    )}
                  </div>
                </button>
              ))}
            ...
            {/* Visual Scan Effect */}
            <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-transparent via-brand-gold/5 to-transparent w-full h-full -translate-x-full animate-[scan_8s_linear_infinite]" />
          </div>
          ...
          <div className="w-full max-w-5xl min-h-[220px] flex items-center justify-center relative px-4">
            <AnimatePresence mode="wait">
              {activeHotspot ? (
                ...
                >
                  <h3 className="text-3xl md:text-4xl font-bold text-brand-gold mb-6 tracking-tight">
                ...
                >
                  <div className="inline-flex items-center gap-4 px-10 py-5 bg-white/5 border border-white/10 rounded-full text-brand-muted text-base md:text-lg hover:bg-white/10 transition-colors cursor-default">
                    <Info className="w-6 h-6 text-brand-gold animate-bounce" />
        ...
        <div className="mb-40 px-4 md:px-8">
          <div className="max-w-4xl mx-auto text-center mb-20 px-4">
            ...
            <p className="text-xl md:text-2xl text-brand-muted leading-relaxed mb-12 font-light">
              {t('conceptToTable.paragraph')}
            </p>
            <div className="inline-block px-10 py-6 bg-brand-gold/10 border-s-4 border-brand-gold rounded-2xl">
              <p className="text-2xl md:text-3xl text-brand-gold font-medium italic">
          ...
          <div className="relative w-full max-w-[1600px] mx-auto aspect-[16/8] md:aspect-[21/9] rounded-[3.5rem] overflow-hidden group border border-white/10 shadow-3xl">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-brand-black via-transparent to-transparent z-10 opacity-90" />
            
            {/* The Blueprint Overlay Reveal */}
            <div className="absolute inset-0 bg-brand-black/95 backdrop-blur-xl transition-all duration-1000 group-hover:opacity-0 group-hover:backdrop-blur-none flex flex-col items-center justify-center z-20">
               ...
               <div className="absolute bottom-16 flex flex-col items-center gap-6">
                  <div className="w-px h-16 bg-gradient-to-b from-transparent to-brand-gold animate-pulse" />
                  <span className="text-brand-gold tracking-[0.5em] text-sm md:text-base uppercase font-bold">
        ...
        <div className="text-center max-w-3xl mx-auto border-t border-white/5 pt-32 px-4">
           ...
           <p className="text-2xl text-brand-gold mb-16 font-light">{t('closing.line')}</p>
           
           <div className="bg-gradient-to-b from-white/5 to-transparent p-12 rounded-[3rem] border border-white/5 shadow-2xl">
             <span className="block text-xs text-brand-gold tracking-[0.4em] uppercase mb-6 font-bold">
               {t('closing.lamassuLabel')}
             </span>
             <p className="text-lg text-brand-muted max-w-md mx-auto leading-relaxed font-light">
