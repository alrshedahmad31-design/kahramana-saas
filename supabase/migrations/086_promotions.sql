-- ============================================================
-- 086_promotions.sql
-- Sprint: Waiter App + QR Ordering + Promotion Engine (2/2)
--
-- Adds:
--   1. promotion_type ENUM (5 strategies)
--   2. promotions table — branch-scoped or global, time-windowed, capped
--   3. orders.promotion_id + orders.promotion_discount_bhd
--   4. rpc_create_order — extended with p_promotion_id + p_promotion_discount_bhd
--      (validates active + window + cap, increments use_count, subtracts from total)
--
-- Roles allowed to manage promotions: owner, general_manager, branch_manager,
-- marketing (matches coupons pattern from 009 / 028).
--
-- SAFE TO RE-RUN.
-- ============================================================
...
DO $$ BEGIN
  CREATE TYPE promotion_type AS ENUM (
    ...
    'bundle',         -- Buy X + Y for fixed price Z
    'time_discount',  -- % off during specific days/times (breakfast, lunch)
    'item_discount',  -- % off a specific item
    'spend_discount'  -- Spend X get Y discount
  );
EXCEPTION
...
CREATE TABLE IF NOT EXISTS promotions (
  ...
  --   bogo:           { "buy_slug":"...", "get_slug":"..." }
  --   bundle:         { "items":["slug1","slug2"], "price_bhd":4.5 }
  --   time_discount:  { "discount_pct":20, "days":[0,6], "start_time":"07:00", "end_time":"11:00" }
  --   item_discount:  { "slug":"...", "discount_pct":15 }
  --   spend_discount: { "min_spend_bhd":5, "discount_pct":10 }
...
CREATE INDEX IF NOT EXISTS idx_promotions_branch_active
  ON promotions (branch_id, is_active)
  WHERE is_active;
...
CREATE TRIGGER trg_promotions_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION promotions_set_updated_at();

-- ── 3. RLS policies ──────────────────────────────────────────

ALTER TABLE promotions ENABLE ROW LEVEL SECURITY;

-- READ: any authenticated staff member sees promotions for their branch + global.
-- Owner / GM see every promotion.
DROP POLICY IF EXISTS "promotions_select_staff" ON promotions;
...
-- INSERT: owner / GM (any branch incl. global) or branch_manager / marketing
-- (own branch only — cannot create global promotions).
DROP POLICY IF EXISTS "promotions_insert_manager" ON promotions;
CREATE POLICY "promotions_insert_manager"
  ON promotions FOR INSERT TO authenticated
  WITH CHECK (
    auth_user_role() IN ('owner', 'general_manager')
    OR (
      auth_user_role() IN ('branch_manager', 'marketing')
      AND branch_id IS NOT NULL
      AND branch_id = auth_user_branch_id()
    )
  );

DROP POLICY IF EXISTS "promotions_update_manager" ON promotions;
...
DROP POLICY IF EXISTS "promotions_delete_manager" ON promotions;
...
-- ── 4. orders.promotion_id + promotion_discount_bhd ──────────

ALTER TABLE orders ADD COLUMN IF NOT EXISTS promotion_id UUID
  REFERENCES promotions(id) ON DELETE SET NULL;

ALTER TABLE orders ADD COLUMN IF NOT EXISTS promotion_discount_bhd NUMERIC(10,3)
  NOT NULL DEFAULT 0
  CHECK (promotion_discount_bhd >= 0);

CREATE INDEX IF NOT EXISTS idx_orders_promotion
  ON orders (promotion_id)
  WHERE promotion_id IS NOT NULL;

-- ── 5. rpc_create_order — extended ───────────────────────────
-- Changes vs 085:
--   + p_promotion_id UUID + p_promotion_discount_bhd NUMERIC params
--   + validation: active, within window, under max_uses
--   + increments use_count atomically
--   + subtracts promotion_discount_bhd from final total (alongside coupon + points)
--   + INSERT writes promotion_id + promotion_discount_bhd
