-- Migration 113: Strict Shawarma Routing
-- Shawarma station is separate and should ONLY contain Meat and Chicken Shawarma.
-- All other sandwiches (Liver, Kubba, etc.) move to 'mains' if not 'grill'.
...
BEGIN
  ...
  v_station := CASE
    ...
    -- 3. Shawarma (Strictly Meat and Chicken Shawarma only)
    WHEN v_slug LIKE 'shawarma-%'   OR v_name LIKE '%شاورما%' THEN 'shawarma'
    WHEN v_slug LIKE 'sandwiches-%' AND v_name LIKE '%شاورما%' THEN 'shawarma'
    WHEN v_name LIKE '%صاج%'        OR v_name LIKE '%شباتي%' THEN 'shawarma'

    -- 4. Mains (Everything else cooked in kitchen: Stews, Rice, Eggs, and NON-Shawarma Sandwiches)
    ...
    WHEN v_slug LIKE 'sandwiches-%' THEN 'mains' -- Catch-all for sandwiches (Liver, Kubba, etc.)
