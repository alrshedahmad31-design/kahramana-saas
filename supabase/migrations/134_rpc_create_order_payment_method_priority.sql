-- ============================================================
-- Kahramana Baghdad
-- Migration: 134_rpc_create_order_payment_method_priority.sql
--
-- VULN-104: rpc_create_order status-derivation CASE was source-first.
-- A cashier could create a manual + tap order and the function would
-- match `p_source = 'manual'` first, returning status='accepted' even
-- though no Tap transaction has been authorised. The order would skip
-- the 'pending_payment' state entirely, the 30-minute expires_at would
-- not be stamped, and the webhook reconciler had no row to settle.
--
-- Fix:
--   Invert CASE priority — check payment_method FIRST:
--     1. payment_method='tap'                            -> pending_payment
--     2. payment_method='cash' + source IN(manual,waiter,qr)
--                                                        -> accepted
--     3. source IN(manual,waiter,qr) with other methods  -> existing logic
--                                                           (accepted)
--     4. payment_method IN(benefit_pay,online)           -> pending_payment
--     5. fall-through                                    -> new
--
-- Function signature, parameter list, GRANTS, and all other behaviour
-- are preserved verbatim from migration 091. Only the CASE statement
-- on v_status is changed.
--
-- ROLLBACK:
--   Re-run the function definition from migration 091.
-- ============================================================
