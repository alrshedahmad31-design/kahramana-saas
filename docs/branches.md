# Branch Contacts — Kahramana Baghdad
> Source of truth for all branch contact data.
> Last verified: 2026-04-27 (from live site audit)
> Any update here MUST be reflected in `src/constants/contact.ts`

---

## فرع الرفاع — الحجيات

| Field | Value |
|---|---|
| Name (AR) | فرع الرفاع — الحجيات |
| Name (EN) | Riffa Branch — Al-Hijiyat |
| Address (AR) | منطقة الحجيات، الرفاع، البحرين |
| Address (EN) | Al-Hijiyat Area, Riffa, Bahrain |
| City (AR) | الرفاع |
| City (EN) | Riffa |
| Phone | +973 1713 1413 |
| WhatsApp | +973 1713 1413 |
| wa.me link | https://wa.me/97317131413 |
| Google Maps | TODO — confirm with restaurant |
| Hours (AR) | يومياً ٧:٠٠م – ٢:٠٠ص |
| Hours (EN) | Daily 7:00 PM – 2:00 AM |
| Opens | 19:00 |
| Closes | 02:00 |
| Delivery | Yes |
| Dine-in | Yes |

---

## فرع المحرق — قلالي

| Field | Value |
|---|---|
| Name (AR) | فرع المحرق — قلالي |
| Name (EN) | Muharraq Branch — Qallali |
| Address (AR) | الشارع الرئيسي، قلالي، البحرين |
| Address (EN) | Main Street, Qallali, Bahrain |
| City (AR) | المحرق |
| City (EN) | Muharraq |
| Phone | +973 1713 1213 |
| WhatsApp | +973 1713 1213 |
| wa.me link | https://wa.me/97317131213 |
| Google Maps | TODO — confirm with restaurant |
| Hours (AR) | يومياً ١٢:٠٠م – ١:٠٠ص |
| Hours (EN) | Daily 12:00 PM – 1:00 AM |
| Opens | 12:00 |
| Closes | 01:00 |
| Delivery | Yes |
| Dine-in | Yes |

---

## General Contact

| Field | Value |
|---|---|
| Email | info@kahramanat.com |
| Website | https://kahramanat.com |
| Instagram | https://www.instagram.com/kahramanat_b |
| TikTok | https://www.tiktok.com/@kahramanat_b |
| Snapchat | @kahramanat_b |
| Facebook | https://www.facebook.com/kahramanat1 |

---

## Planned Branch (Phase 3+)

| Field | Value |
|---|---|
| Name (AR) | فرع البديع |
| Name (EN) | Al-Badi' Branch |
| Status | Planned — no opening date confirmed |

---

## Notes

- WhatsApp numbers confirmed via live site audit 2026-04-27 — need official sign-off from restaurant
- Riffa opens at 7pm, Qallali opens at 12pm (noon) — different hours, must be handled per-branch
- Google Maps links: TBD — restaurant to provide or confirm via Google Business Profile
- All phone numbers stored as env vars in Supabase — never hardcoded in source
